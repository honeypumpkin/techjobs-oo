...
//        if (! jobForm.getName().equals("")) {
        if (errors.hasErrors()) {
            ...
            CoreCompetency coreCompetencies = jobData.getCoreCompetencies().findById(jobForm.getCoreCompetenciesId());
            PositionType positionType = jobData.getPositionTypes().findById(jobForm.getPositionTypesId());

            Job newJob = new Job(name, employer, location, positionType, coreCompetencies);
            jobData.add(newJob);
